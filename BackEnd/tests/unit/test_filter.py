...
class TestFilterVerification(unittest.TestCase):
    ...
    def test_verify_room_type(self):
        ...
        self.assertFalse(self.filter.verify_room_type(
            {"Bedrooms": [], "Bathrooms": 9}))
        self.assertFalse(self.filter.verify_room_type(
            {"Bedrooms": 4, "Bathrooms": ""}))

        # testing false for invalid bounds
        self.assertFalse(self.filter.verify_room_type(
            {"Inavlid Key Name 1": "1", "Invalid Key Name 2": "1"}))
        self.assertFalse(self.filter.verify_room_type(
            {"Bedrooms": "2.5", "Bathrooms": "3"}))
        self.assertFalse(self.filter.verify_room_type(
        ...
        self.assertTrue(self.filter.verify_room_type(
    ...
    def test_verify_availability(self):
        ...
        # testing false for invalid bounds
        self.assertFalse(self.filter.verify_availability(
            {1: "key not str", "Year": "2020"}))
        self.assertFalse(self.filter.verify_availability(
            {"Month": "January", "Year": 2020}))
        self.assertFalse(self.filter.verify_availability(
            {"Not Month": "January", "Year": "1999"}))
        self.assertFalse(self.filter.verify_availability(
            {"Month": "December", "year": "2020"}))
        self.assertFalse(self.filter.verify_availability(
            {"Month": "Jason Derulo", "Year": "2020"}))
        self.assertFalse(self.filter.verify_availability(
            {"Month": "December", "Year": "twenty twenty"}))
        self.assertFalse(self.filter.verify_availability(
            {"Month": "December", "Year": "5"}))

        # testing true for valid inputs
        self.assertTrue(self.filter.verify_availability(
