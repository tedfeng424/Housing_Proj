...
#multiple criteria
INPUT_TYPE_AND_PRICE = {'room_type' : {'Bedrooms' :NUM_BED, 'Bathrooms' : NUM_BATH}, 'rent' : [1000, 3000]}
INPUT_PRICE_AND_DIST = {'rent' : [1000, 3000], 'distance' : "<25 mins"}
INPUT_TRIPLE1 = {'room_type' : {'Bedrooms' : '3+', 'Bathrooms' : '2'}, 'rent' : [3000, 5000], 'distance' : '<40 mins'}
INPUT_TRIPLE2 = {'room_type' : {'Bedrooms' : '3+', 'Bathrooms' : '3+'}, 'distance' : '<30 miins', 'availability' : {'Year' : '2023', 'Month' : 'September'}}
INPUT_ALL_CRITERIA = {'room_type' : {'Bedrooms' :NUM_BED, 'Bathrooms' : NUM_BATH}, 'rent' : [LOWER_PRICE, UPPER_PRICE], "distance" : DISTANCE, "availability" : {"Year" : '2024', "Month" : "June"}}
...
class TestFilterDb(unittest.TestCase):
    ...
    def test_invalid_price(self):
        filter_range = filter_db.DBFilter(INPUT_INVALID_PRICE_RANGE,self.session)
        results = filter_range.filter()
        self.assertEqual(20, len(results))

        filter_type = filter_db.DBFilter(INPUT_INVALID_PRICE_TYPE, self.session)
        results = filter_type.filter()
        self.assertEqual(20, len(results))

    def test_invalid_availability(self):

        filter_obj = filter_db.DBFilter(INPUT_INVALID_AVAILABILITY1,self.session)
        results = filter_obj.filter()
        self.assertEqual(20, len(results))

        filter_obj = filter_db.DBFilter(INPUT_INVALID_AVAILABILITY2, self.session)
        results = filter_obj.filter()
        self.assertEqual(20, len(results))

        filter_obj = filter_db.DBFilter(INPUT_INVALID_AVAILABILITY3, self.session)
        results = filter_obj.filter()
        self.assertEqual(20, len(results))

        filter_obj = filter_db.DBFilter(INPUT_INVALID_AVAILABILITY4, self.session)
        results = filter_obj.filter()
        self.assertEqual(20, len(results))
