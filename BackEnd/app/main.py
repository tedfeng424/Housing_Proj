...
def newRoom():
    ...
    room = Room(user_id=1, intro=intro,
                price_range=price_range, stay_period=stay_period,
                move_time=move_time,
                name=name, lat=requested_json["latlng"]["lat"],
                lng=requested_json["latlng"]["lng"])
    session.add(room)
    session.commit()
    # load pictures in db
    main_p = PictureRoom(
        picture=main_pic, type_n="main_room", room=room, user_id=1)
    session.add(main_p)
    session.commit()
    for p in pics:
...
def showRoom():
    ...
    response = []
    for room in rooms:
        t_main = session.query(PictureRoom).filter_by(
            room_id=room.id, type_n="main_room").one().picture
...
def getRoom():
    ...
    pics_q = session.query(PictureRoom.picture).filter_by(
        room_id=room_id).all()
...
def showPpl():
    ...
    response = []
    for t in tenants:
        t_main = session.query(PicturePerson).filter_by(
            tenant_id=t.id).one().picture
