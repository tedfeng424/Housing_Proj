...
export default class HouseDetail extends React.Component{
    state = {name:"",id:"",intro:"",main_pic:"",pics:[],price_range:"",stay_period:"",move_time:"",others:[],lat:0, lng:0,markers:[],IMAGES:[],thumbnailWidth_big:200,thumbnailWidth_small:100,thumbnailHeight:180}
    updateDimensions() {
        this.setState({thumbnailWidth_small:Math.floor(($('#gallery').width())/3),
        thumbnailWidth_big:Math.floor(($('#gallery').width())/2),
        thumbnailHeight:Math.floor($('#gallery').height()/2)-2,
     },()=>this.setState({IMAGES:this.state.pics.map((pic, index) => {
        return (
            {
                src: pic,
                thumbnail: pic,
